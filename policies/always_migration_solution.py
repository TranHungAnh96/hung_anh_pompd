...
def calculate_cost_at_each_time_step(env,
    ...
    action = int(user_position_index)
    ...
    communication_migration_cost = env._get_number_of_hops(service_index,
                                                           action) * migration_coefficeitn + migration_cost + \
...
def always_migration_solution(env, system_infos):
    ...
    always_migration_rewards = np.mean(np.sum(cost_batch, axis=-1))
