...
def calculate_cost_at_each_time_step(env,
    ...
    migration_coefficient = system_infos[time_step][-1]

    edge_weights = []
    computation_costs = []
    communication_costs=[]

    action = np.random.randint(0, env._action_dim)
    ...
    communication_migration_cost = env._get_number_of_hops(service_index, action) * migration_coefficient + migration_cost + \
...
def random_solution(env, system_infos):
...
if __name__ == "__main__":
  ...
  8, 11, 12, 19, 11,  9,  5, 16,  9,  8, 10, 12, 20, 16,  8]
    ...
    random_latency = random_solution(env, system_infos)
    print("random_latency is: ", random_latency)
