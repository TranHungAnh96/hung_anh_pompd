...
logger.configure(dir="./log/test_request_pattern_san", format_strs=['stdout', 'log', 'csv'])
...
for density in density_set:
    ...
    _, system_infos = am_eval_sampler.obtain_samples(is_rnn=False)
    ...
    print("avg AM time is: ", (time.time() - begin_time) / 30.0)
    ...
    print("avg NM time is: ", (time.time() - begin_time) / 30.0)
    ...
    print("avg RM time is: ", (time.time() - begin_time) / 30.0)
    begin_time = time.time()
    dqn_rewards_collects, _ = q_network_eval_sampler.obtain_samples(is_rnn=False)
    print("avg DQLM inference time is: ", (time.time() - begin_time) / 30.0)


    begin_time = time.time()
    drqn_rewards_collects, _ = rnn_q_net_sampler.obtain_samples(is_rnn=True)
    print("avg DRQLM inference time is: ", (time.time() - begin_time) / 30.0)
    ...
    print("avg DRACM time is: ", (time.time() - begin_time) / 30.0)
    ...
    print("avg MABTS time is: ", (time.time() - begin_time) / 30.0 / 3.0)
