...
EnvironmentParameters = namedtuple('ServersParameter', ['trace_start_index', 'num_traces','num_base_station', 'optical_fiber_trans_rate',
                                                        'server_result_data_lower_bound','server_result_data_higher_bound',
                                                        
                                                       'server_frequency',  'ratio_lower_bound','ratio_higher_bound',
...
class MECServer(object):
    def __init__(self, index, poisson_rate, task_data_lower_bound, task_data_higher_bound,result_data_lower_bound,
                 result_data_higher_bound,ratio_lower_bound, ratio_higher_bound, frequence = 32.0):
        self.poisson_rate = poisson_rate
        self.index = index

        self.task_data_lower_bound = task_data_lower_bound
        self.task_data_higher_bound = task_data_higher_bound
        self.result_data_higher_bound = result_data_higher_bound
        self.result_data_lower_bound = result_data_lower_bound 
...
class BatchMigrationEnv(gym.Env):
    def __init__(self, env_parameters):
        ...
        #del self.migration_coefficient_low = env_parameters.migration_coefficient_low
        #del self.migration_coefficient_high = env_parameters.migration_coefficient_high


        #del self.backhaul_coefficient = env_parameters.backhaul_coefficient
    ...
    #del def get_migration_coefficient(self):
    #del    return np.random.uniform(self.migration_coefficient_low, self.migration_coefficient_high)
    ...
    def _make_state_according_to_action(self, trace_id, action):
        ...
        #del current_migration_coefficient = self.get_migration_coefficient()
        #del self._migration_coefficient[trace_id] = current_migration_coefficient
        ...
        for server in self.server_list:
            ...
            wired_communication_cost = (task_data_volume / self._optical_fiber_trans_rate) * min(num_of_hops, 1)  #del + self.backhaul_coefficient * num_of_hops
            ...
            communication_cost = float(task_data_volume) / float(trans_rate) + wired_communication_cost + \
                                current_migration_cost * min(migration_num_of_hops,1) #del+change (migration_num_of_hops * current_migration_coefficient) + current_migration_cost)
    ...
    def reset(self):
        ...
        #del self._migration_coefficient = np.zeros(shape=(self._num_traces,), dtype=np.float32)
